...
create table if not exists category(id serial primary key,label varchar (50),parentcategoryid integer);
create table if not exists products (id serial primary key,label varchar (50),price int ,categoryid integer,stock integer,
                                     constraint fk_customer foreign key (categoryid) references category(id));
