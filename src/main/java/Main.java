...
public class Main {
    ...
    public static Category choosingCategory() {
        ...
        while (condition) {
            ...
            try {
                ...
                switch (operator) {
                    case 1:
                        category = addingNewCategory();
                        break;
                    case 2:
                        category = selectingFromCurrentCategory();
                        break;
                }
    ...
    public static void customerLogin() {
        try {
            ...
            System.out.println("please enter your password");
            String password = input.next();
            Customer customer = new Customer(0l, "", "", "", "", 0l, "", 0l, 0l);
    ...
    public static void loggedInCustomer(Customer customer) {
        ...
        while (condition) {
            ...
            try {
                ...
                switch (operator) {
                    case 1:
                        customer1 = addingProductToCart(customer);
                        break;
                    case 2:
                        Cart cart = new Cart();
                        cart.setId(Integer.valueOf(String.valueOf(customer.getCartId())));
                        cart.setProductList(cartProductList);
                        customer1 = deletingProductFromCart(customer);
                        break;
                    case 3:
                        submitCart(customer);
                        cartProductList = new ArrayList<>();
        ...
        try {
            ...
            for (Product product : products
                ...
                if (Objects.equals(product.getId(), productId)) {
                    ...
                    cartRepository.findById(Integer.valueOf(String.valueOf(customer.getCartId()))).setProductList(cartProductList);
                    return customer;
                }
            }
            System.out.println("there is no product with this id!");
    ...
    public static Customer deletingProductFromCart(Customer customer) {
        List<Product> products = cartProductList;
        for (Product product : products
        ) {
    ...
    public static void submitCart(Customer customer) {
        try {
            ...
            cart.setCostumer(customer);
            cart.setProductList(cartProductList);
            Integer id = cartRepository.save(cart);
            customer.setCartId(Long.valueOf(String.valueOf(id)));
        } catch (InputMismatchException e) {
            System.out.println("please enter a number!");
            input.nextLine();
        }
